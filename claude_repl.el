...
(defun claude-repl-send-to-redis (message)
  ...
  (let ((timestamp (format-time-string "%s")))
    ...
    ;; Store message in Redis stream for bridge to pick up (correct stream name)
    (let ((redis-result (shell-command-to-string
      ...
      (message "Redis send result: %s" redis-result)
      redis-result)
...
(defun claude-repl-check-bridge-responses ()
  ...
  (let ((responses (shell-command-to-string "redis-cli XRANGE claude:responses - + COUNT 10")))
    (when (and responses 
      ...
      ;; Keep conversation history - don't delete responses
  ...
  (let ((lines (split-string responses-data "\n")))
    (dotimes (i (length lines))
      (when (and (< i (length lines))
                 (string= (nth i lines) "response")
                 (< (+ i 1) (length lines)))
        ;; Found "response" field, next line contains the actual message
        (let ((response-text (nth (+ i 1) lines)))
...
(defun claude-repl-display-bridge-response (response)
  ...
  (with-current-buffer claude-repl-buffer
    ...
    (save-excursion
      (when (re-search-backward "Claude is thinking\\.\\.\\." nil t)
        (beginning-of-line)
        (kill-line 2)))  ; Kill "thinking..." line and the empty line after
    ;; Insert the formatted response directly 
    (insert "\n" (propertize response 'face 'claude-repl-claude-face) "\n")
    (goto-char (point-max))))
